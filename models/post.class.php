	...
	class Post extends Contributo{
		...
		* ID, titolo, autore, descrizione (opzionale), data       */
		public function get_posts_meta(){
			...
			$meta = $f->get_posts_meta();
			return $meta;
		}
	
		public function get_drafts_meta(){
			$f = new DBController($this->dbms);
			$meta = $f->get_drafts_meta();
			return $meta;
		}
		...
		public function salvaPost($idContr,$isdraft,$idTesto,$idRisorsa){
			$f = new DBController($this->dbms);
			$f->salvaPost($idContr,$isdraft,$this->getInfoUDA(),$idTesto,$idRisorsa,$this->getInfoNodo());
		}
